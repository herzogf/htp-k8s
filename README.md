...
htp-k8s is a single binary (the web UI is embedded) that serves a browser UI against whatever cluster your current kubeconfig points at. There's no authentication anywhere in it (it's a read-only viewer, see below), so **it listens on `127.0.0.1:8080` — loopback only — by default**: reachable from a browser on the same machine, not from anywhere else on the network. `-addr :8080` (or `HTP_K8S_ADDR=:8080`) opens that up — but on its own that only gets a remote browser a *loaded page with no data*: the released frontend's WebSocket target is baked to `ws://localhost:8080/ws` regardless of where the page was loaded from, so it dials the *viewer's own* machine, not htp-k8s's. Real remote viewing additionally needs the frontend rebuilt with a matching `VITE_WS_URL` — see [Running & developing locally](docs/running-locally.md#gotchas) — which has always been true, not something this change adds.
...
`-e HTP_K8S_ADDR=:8080` is required here — it's not optional the way it looks. Docker's `-p 127.0.0.1:8080:8080` forwards traffic to the *container's* own interface address, never to its loopback, so a container left on the loopback-only default above would never see that traffic at all. The `-p` host-side binding (loopback) is what actually keeps this off your network, exactly as it does for the bare binary above — the container just has to bind wider *inside its own network namespace* for that forwarding to reach it. (Baking a wider default into the image binary itself — an image-only build-time default — is mechanically possible, but deliberately not done: it would make the `--network host` fallback recipe below fail *open* instead of closed if someone forgets its own `-e HTP_K8S_ADDR=127.0.0.1:8080` — every other way to forget a flag in this design loses connectivity, not access control, and that's the one shape that wouldn't. Separately, there's no way to set this via a container-*runtime* env var baked into the image: ko, which builds it, only supports Go *build*-time env through its config — verified against the pinned ko and GoReleaser versions while implementing this.)
...
```bash
# NOTE: HTP_K8S_ADDR is "127.0.0.1:8080" here, NOT the bare ":8080" used
# above — --network host makes docker's `-p` a no-op, so the app must bind
# loopback ITSELF or it's reachable from every interface on this host.
