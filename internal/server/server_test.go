...
// TestWS_SendsPlaceholderMessage is the end-to-end proof that a client
// connecting to /ws over a real socket (not an in-memory recorder, since
// WebSocket upgrades need an actual hijackable connection) receives a
// non-empty placeholder text message — the backend half of the walking
// skeleton (issue #4).
func TestWS_SendsPlaceholderMessage(t *testing.T) {
	ts := httptest.NewServer(server.NewHandler())
	defer ts.Close()
	...
	if len(body) == 0 {
		t.Fatal("message body is empty, want non-empty placeholder text")
	}
}
