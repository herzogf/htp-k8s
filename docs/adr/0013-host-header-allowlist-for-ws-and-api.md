...
- **An IP-address-literal `Host`** (v4 or v6). DNS rebinding requires resolving a *name*; a `Host` that's already a bare IP was never routed through DNS, so there's no name to rebind. This is also what makes a wildcard `-addr :8080` bind — the only option inside a container — reachable by a browser at its IP address with zero extra configuration, matching the "#160 must not reinstate a configuration step" constraint the original grill on issue #163 established. Concretely: `README.md` has three `docker run` recipes, and the two that publish a port with Docker's `-p` (the remote-cluster and local-kind recipes) both use `-e HTP_K8S_ADDR=:8080` — a wildcard bind. (The third, `--network host`, deliberately uses `HTP_K8S_ADDR=127.0.0.1:8080` instead, since `--network host` makes `-p` a no-op and the app must bind its own loopback — see that recipe's inline `NOTE` in `README.md`; it isn't the wildcard-bind case this bullet is about.)
