...
- **Glance stream decoupling.** Seeded aim "glances" now draw from a separate PRNG stream derived from the same seed, so aim-side changes can no longer shift the waypoint sequence — the logged-seed reproduction promise rests on the walk alone from here on.
- Both changes mean a given seed's realized route differs from earlier builds — within this ADR's reproduction contract (deterministic per version, not stable across versions).
