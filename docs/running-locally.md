...
| Flag | Env | Purpose |
| --- | --- | --- |
| `-addr` | `HTP_K8S_ADDR` | Listen address; default `127.0.0.1:8080` (**loopback only** — issue #127: there's no auth layer, so nothing is reachable off this machine unless you opt in). Pass `-addr :8080` to widen it — but that alone only reaches `/api` directly (curl, custom tooling); a remote *browser* additionally needs the frontend rebuilt with `VITE_WS_URL`, same as changing the port does. See the `/ws` gotcha below. |
