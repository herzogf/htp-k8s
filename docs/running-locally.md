...
| `-allowed-hosts` | `HTP_K8S_ALLOWED_HOSTS` | Comma-separated extra hostnames `/ws` and `/api` trust in the HTTP `Host` header (any port), beyond loopback and the host `-addr` names when it's a concrete IP — both of those are trusted with no configuration. Needed for a reverse proxy or a wildcard `-addr` bind (`:8080`), which can't derive its own reachable name automatically; see [ADR-0013](adr/0013-host-header-allowlist-for-ws-and-api.md). |
